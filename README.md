...
- [1. Тема и целевая аудитория](#1-тема-и-целевая-аудитория)
  ...
  - [Целевая аудитория](#целевая-аудитория)
    - [Распределение пользователей по возрасту](#распределение-пользователей-по-возрасту)
    - [Распределение пользователей по странам](#распределение-пользователей-по-странам)
  - [Статистика контента](#статистика-контента)
- [2. Расчет нагрузки](#2-расчет-нагрузки)
  - [Продуктовые метрики](#продуктовые-метрики)
    - [Среднее количество действий пользователя по типам в день по странам](#среднее-количество-действий-пользователя-по-типам-в-день-по-странам)
  ...
  - [Технические метрики](#технические-метрики)
    ...
    - [Ожидаемый прирост](#ожидаемый-прирост)
- [3. Глобальная балансировка нагрузки](#3-глобальная-балансировка-нагрузки)
  - [Расположение дата центров](#расположение-дата-центров)
  - [Алгоритм глобальной балансировки](#алгоритм-глобальной-балансировки)
- [4. Локальная балансировка нагрузки](#4-локальная-балансировка-нагрузки)
  - [Схема балансировки для входящих и межсервисных запросов](#схема-балансировки-для-входящих-и-межсервисных-запросов)
  - [Схема отказоустойчивости](#схема-отказоустойчивости)
  - [Нагрузка по терминации SSL](#нагрузка-по-терминации-ssl)
- [5. Логическая схема БД](#5-логическая-схема-бд)
  - [Логическая схема данных](#логическая-схема-данных)
  - [Размер данных](#размер-данных)
  - [Нагрузка на чтение/запись](#нагрузка-на-чтениезапись)
- [6. Физическая схема БД](#6-физическая-схема-бд)
  - [Хранение основных данныъ](#хранение-основных-данныъ)
    - [Индексирование](#индексирование)
    - [Шардинг](#шардинг)
    - [Репликация](#репликация)
  - [Кеширование данных](#кеширование-данных)
  - [Резервное копирование](#резервное-копирование)
  - [Хранение файлов приложений](#хранение-файлов-приложений)
...
| Таблица       | Величина                       |
| ------------- | ------------------------------ |
| `User`        | `500e6 * 50 KB = 25 TB`        |
| `Application` | `3.7e6 * 1 KB = 3.7 GB`        |
| `AppVersion`  | `3.7e6 * 3 * 100 KB = 1.11 TB` |
| `Review`      | `967 * 3.7e6 * 1 KB = 3.5 TB`  |
| `Rating`      | `3.7e6 * 10 B = 37 MB`         |

Итого, общий объем данных в бд составить примерно `30 TB`, не считая сами приложения

Сами приложения будем хранить отдельно с использованием распределенной системы хранения объектов (S3).
...
В качестве схемы репликации будем использовать стандартную схему с физической репликацией. Одна база данных на запись (`master`) и много на чтение (`slave`), так как запросов на чтение в нашем приложении будет на несколько порядков больше, чем на запись.

![replication](images/db_replication.png)

### Кеширование данных

Для кеширования часто запрашиваемых данных и снижения нагрузки на основную базу данных будем использовать `Redis`.
...
### Хранение файлов приложений

Для хранения неструктурированных данных (файлы приложений) будем использовать `S3` хранилища. Подобное хранилище обеспечивает [^14]:
...
- **Версионность** - хранилище S3 позволяет настроить механизм хранения версий любых объектов, что в нашем случае крайне необходимо
